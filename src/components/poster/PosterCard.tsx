...
import posterEmptyImage from '@/../public/general/img-poster-empty.png';
      ...
      {posters.map((poster) => {
        ...
        return (
          <Spin key={_id} spinning={isPostersLoading}>
            <Card
              ...
              cover={
                <div className="h-52 w-full items-center overflow-hidden">
                  {<Image
                    src={getImageId(link) ? `https://drive.google.com/uc?export=view&id=${getImageId(link)}` : posterEmptyImage}
                    alt="poster_Image"
                    width={300}
                    height={215}
            ...
            >
              <Meta
                title={<div className="flex text-wrap text-lg font-bold justify-center">{title}</div>}
              />
            </Card>
          </Spin>
        );
